...
Run a single test file: `npx vitest run src/ridership/lineMetrics.test.ts` (or pass a name filter with `-t`). Tests use Vitest + `@testing-library/react` in a jsdom environment with globals enabled (no per-file imports of `describe`/`it`/`expect` needed).
...
2. **Records are consolidated by line** — [src/App.tsx](src/App.tsx) fetches `/ridership.json` at runtime (a columnar blob decoded by [src/utils/ridershipData.ts](src/utils/ridershipData.ts)), then hands it to one `buildRidershipView(...)` call in a `useMemo`. That call returns `{ months, datasets, consolidated, events, metrics, coverage }` — the last two keyed by line id, so the per-line figures are already derived by the time anything renders. The derivation itself — filtering to the selected date window, grouping records by `line_name` into `ConsolidatedRidership`, and building the shared month axis, Chart.js datasets and event list — lives in [src/ridership/buildRidershipView.ts](src/ridership/buildRidershipView.ts); the module's only public surface is [src/ridership/index.ts](src/ridership/index.ts). It yields an empty view until the fetch resolves.
3. **Figures are joined onto lines as Line Readouts** — `buildLineReadouts({ lines, metrics, coverage })` in App's `useMemo` (~L98) produces one `LineReadout` per line: the `Line` with the figures *this* window derived, spread over it. `listedReadouts()` (~L103) narrows that to the rows the table shows. `LineSelector`, `LineTableRow`, `SummaryData`, `Map` and `mapPopup` all take `LineReadout[]`, not `Line[]`. The figures come from `lineMetrics()` in [src/ridership/lineMetrics.ts](src/ridership/lineMetrics.ts) — one function returning one nullable shape, replacing the five `src/utils/calc.ts` helpers it was split across (see [docs/adr/0004-line-metrics-are-one-nullable-shape.md](docs/adr/0004-line-metrics-are-one-nullable-shape.md)).

   **The write-back is still there, and is still on the way out.** `updateLinesWithLineMetrics()` (in the hook) runs from a `useEffect` at `App.tsx:113-116` and stamps the same eight fields onto every `Line`, minting a fresh `lines` array that re-enters the memo it came from — which is what the `JSON.stringify` dependency keys below exist to contain. Nothing renders those stamped values any more (the readout spread overwrites them), but they are not dead: `isVisibleLine` in the hook gates on `averageRidership !== undefined`, and `selectAllVisibleLines` runs through it. [ADR-0005](docs/adr/0005-derived-figures-live-on-line-readouts.md) accepted deleting the round trip; #154 landed the consumer half only. Prefer a readout over adding another derived field to `Line`.
...
- **All UI state syncs to URL query params** (`start`, `end`, `day`, `lines`, `q`, `buses`, `trains`, `aggregate`, `logs`) so views are shareable. The canonical set lives in [src/hooks/useUserDashboardInput.ts](src/hooks/useUserDashboardInput.ts) — read from the URL on mount in lazy `useState` initialisers (~L92-132) and written back via `history.replaceState` in a `useEffect` (~L150-168); [src/utils/queryParams.ts](src/utils/queryParams.ts) only holds the parse/format helpers. When adding new dashboard state, wire it through both the init readers and the sync effect.
...
- **Month indexing is off by one on purpose** in the record filter, which now lives in [src/ridership/buildRidershipView.ts](src/ridership/buildRidershipView.ts) (`new Date(year, month)` treats month as 0-based while the data is 1-based). Written out: a record at month ordinal `R` is included when `S ≤ R ≤ E − 2`. Preserved from the original implementation and pinned by the committed chart baselines; don't silently change it (see [docs/adr/0001-ridership-month-window-is-deliberately-offset.md](docs/adr/0001-ridership-month-window-is-deliberately-offset.md)). The **Event** filter in the same function is inclusive on both ends — the two windows genuinely disagree, and that is kept, not reconciled.
