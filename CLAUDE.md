...
- **Edit mode**: `isEditMode` lives in App, is exposed through `DockLayoutContext`, and is toggled from the Header. Transient by design — not persisted and not in the URL. Its effect is in [src/dock/MetroTab.tsx](src/dock/MetroTab.tsx): a single-panel group renders a zero-height blank tab instead of a real one, kept in the DOM only because its `data-metro-panel` attribute is the CSS hook `dockTheme.css` styles groups through; edit mode (or a group holding 2+ panels) restores real titled tabs. Since dockview's drag source is the tab element, edit mode is also the only way to drag a lone panel. Close buttons are never rendered — the Header's Panels dropdown is the one owner of panel visibility.
