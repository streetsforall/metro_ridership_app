...
3. **Summary metrics are attached back to lines** — `updateLinesWithLineMetrics()` (in the hook) runs from a `useEffect` in App and stamps average/change/start/end ridership, riders-per-mile and the covered span onto every `Line`. The figures come from `lineMetrics()` in [src/ridership/lineMetrics.ts](src/ridership/lineMetrics.ts) — one function returning one nullable shape, replacing the five `src/utils/calc.ts` helpers it was split across (see [docs/adr/0004-line-metrics-are-one-nullable-shape.md](docs/adr/0004-line-metrics-are-one-nullable-shape.md)). The `LineSelector`/`SummaryData` components read the stamped fields.

   **This step is on the way out.** Step 2 already returns the same figures as `metrics`/`coverage`, so the write-back mints a fresh `lines` array purely to carry data the caller was handed — which is what the `JSON.stringify` dependency keys below exist to contain. [ADR-0005](docs/adr/0005-derived-figures-live-on-line-readouts.md) accepted replacing it with a Line Readout; `buildLineReadouts` and `listedReadouts` are written and unit-tested but not yet imported by anything that renders. Prefer them over adding another derived field to `Line`.
