...
that captured them, so a run on Windows writes `-win32.png` and a run on Linux writes
`-linux.png`. This produces the Linux set without needing a Linux machine.

The whole Playwright suite is gitignored and runs only on a developer machine, so keeping
a Linux set is now a convenience for comparing across machines rather than something CI
reads. See the Testing section of [README.md](../README.md).
...
The image tag is read from `package-lock.json`, so the browser build that writes a
baseline here follows the installed `@playwright/test` and cannot drift from it.
...
When and why you'd run this is covered in the Testing section of
[README.md](../README.md).
